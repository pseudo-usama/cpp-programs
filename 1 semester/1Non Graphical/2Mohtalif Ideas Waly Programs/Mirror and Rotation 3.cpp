// Mirror and Rotation 3.cpp : Defines the entry point for the console application.
//
...
int main()
{
	int selector, rotater = 0, Xmirror = 1, Ymirror = 1, remember0 = 0, remember;
	...
	do{
		for (int y = 5; y >= -5; y--)
		{
			for (int x = -5; x <= 5; x++)
			{
				if (remember0 && Ymirror % 2 == 0) { y = -y; }
				if (remember0 && Xmirror % 2 == 0) { x = -x; }
		...
		cout << "To rotate image in LEFT  side at 90 Degree enter       '4'" << endl;
		cout << "          Enter here       =     ";
		cin >> selector;
		...
		if (selector == 1) { Ymirror = Ymirror + 1; if (rotater == 0) { remember0 = 1; } }
		else if (selector == 2) { Xmirror = Xmirror + 1; if (rotater == 0) { remember0 = 1; } }
